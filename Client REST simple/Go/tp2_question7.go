...
func httpPostRequest(client *http.Client, requestUrl string, msg string) {
...
func httpDeleteRequest(client *http.Client, requestUrl string, msgId string) {

	req, errorMessage := http.NewRequest("DELETE", requestUrl+msgId, nil)
...
func main() {
	...
	for true {
		fmt.Println("----- MENU -----")
		fmt.Println("a - Post new message")
		fmt.Println("b - Delete message from the chat")
		fmt.Println("c - View 50 recent messages and check for new messages every 10 seconds")
		fmt.Println("d - Quit")
		...
		case 'a':
			...
			var message string
			fmt.Scanln(&message)
			httpPostRequest(client, CHAT_URL, message)
		case 'b':
			fmt.Println("Enter a message id : ")
			var messageIdDelete string
			fmt.Scanln(&messageIdDelete)
			httpDeleteRequest(client, CHAT_URL, messageIdDelete)
		case 'd':
	...
	for i := forBeginning; i < len(ids); i++ {
		...
		body, _, _ = getHttpResponse(client, httpGetMessage, "")
	...
	for true {
		...
		if len(body) > 0 {
			ids := bytes.Split(body[lastContentLength:], []byte{byte('\n')})
			...
			for _, id := range ids {
				...
				httpGetMessage := CHAT_URL + string(id)
				body, _, _ = getHttpResponse(client, httpGetMessage, "")
