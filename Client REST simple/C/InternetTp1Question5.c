/*
 ============================================================================
 Name                    : InternetTp1Question5.c
 Author                  : Leonard (Lenny) NAMOLARU
 Source des commentaires : https://curl.se/libcurl/c/libcurl.html
 gcc InternetTp1Question5.c -lcurl
 ============================================================================
 */

#include <stdio.h> // snprintf(), fprintf(), fgets()
...
void post_new_message() { // Source : https://curl.se/libcurl/c/simplepost.html
	  CURL *curl;
	  CURLcode res;

	  char message[INPUT_MAX_LEN + 1];
	  printf("The new message (max : %d) : \n", INPUT_MAX_LEN);
	  // char * fgets (char *s, int count, FILE *stream)
	  fgets(message, INPUT_MAX_LEN, stdin);
...
void delete_message_from_chat() { // Source : https://curl.se/libcurl/c/CURLOPT_CUSTOMREQUEST.html
	  CURL *curl;
	  CURLcode res;

	  char message_id[INPUT_MAX_LEN + 1];
	  printf("Messagde id : \n");
	  // char * fgets (char *s, int count, FILE *stream)
	  fgets(message_id, INPUT_MAX_LEN, stdin);

	  message_id[ID_LENGTH] = '\0';
	  ...
	  if(curl) {
		...
		curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0L);
		 ...
		 int url_length = (strlen(HOST_URL) + ID_LENGTH + 1); // +1 for '\0'
		 char* url = (char *) malloc(sizeof(char) * url_length);

		 // int snprintf (char *s, size_t size, const char *template, ...)
		snprintf(url, url_length, "%s%s", HOST_URL, message_id);

		curl_easy_setopt(curl, CURLOPT_URL, url);
		printf("HTTP DELETE REQUEST TO : %s \n", url);
...
int main(int argc, char* argv[]) {
	...
	do {
		printf("----- MENU ----- \n");
		printf("a - Post new message \n");
		printf("b - Delete message from the chat \n");
		printf("c - View 50 recent messages and check for new messages every 10 seconds \n");
		printf("d - Quit \n");

		printf("Your choise : ");
		scanf("%c", &choise[0]);
		scanf("%c", &choise[1]); // for '\n'

		switch (choise[0]) {
			case 'a' : post_new_message();
			break;
			case 'b' : delete_message_from_chat();
			break;
